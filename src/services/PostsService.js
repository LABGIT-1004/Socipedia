...
class PostsService {
  ...
  static async createPost(formData, token) {
    try {
      ...
      // Check if response indicates blocked content (400 status with description array)
      if (response.status === 400 && response.data && Array.isArray(response.data.description)) {
      ...
      if (error.response && error.response.status === 400 && error.response.data && Array.isArray(error.response.data.description)) {
